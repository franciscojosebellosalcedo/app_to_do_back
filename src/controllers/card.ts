...
export const saveCard=async (req:Request,res:Response)=>{
    try {
        ...
        const cardCreated=await (await newCard.save()).populate(["labels","list"]);
        if(!cardCreated){
...
export const updateCard=async(req:Request,res:Response)=>{
    try{
        ...
        const cardUpdated=await (await Card.findOne({_id:id}))?.populate(["labels","list"]);
...
export const getOneCard=async (req:Request,res:Response)=>{
    try{
        const id=req.params.id;
        const cardFound=await Card.findOne({_id:id}).populate(["labels","list"]);
...
export const getAllCards=async (req:Request,res:Response)=>{
    try{
        const allCards=await Card.find().populate(["labels","list"]);
